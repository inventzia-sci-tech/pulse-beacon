...
public final class TimeMachine {
    ...
    /**
     * Removes a clock-driving gateway and discards any of its events that
     * are still queued. Non-clock-driving gateways are silently ignored.
     *
     * <p>Releases the gateway's write permit so any thread blocked in
    ...
    public void removeGateway(Gateway gateway) {
        ...
        synchronized (this) {
            ...
            queue.removeIf(te -> te.origin() == gateway);
            signalConsumer();
        }
     ...
     * @param topic  the topic the event was published on
     * @param origin the gateway that produced the event
     * @throws InterruptedException if the thread is interrupted while waiting
     *                              for the per-gateway write permit
     */
    public void addEvent(Event event, Topic<?> topic, Gateway origin)
        ...
        synchronized (this) {
            ...
            TimeEvent te = new TimeEvent(event, topic, origin, now, now);
            insertSorted(te);
