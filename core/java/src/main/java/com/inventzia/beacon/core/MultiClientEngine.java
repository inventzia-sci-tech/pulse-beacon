 ...
 * <p>Actors that also implement {@link Runnable} and are registered with
 * {@code ownThread=true} are started on a dedicated daemon thread at startup
 * and stopped at shutdown.
 *
 * <h2>Actor publishing</h2>
 * <p>Actor-to-engine publishing is handled in Phase E when the actor base
 * classes are introduced. For now, {@link #registerActor} wires the engine
 * as the actor's {@link Gateway} reference so actors can call
 * {@link #publish} directly.
 */
public final class MultiClientEngine extends AbstractEngine {
    ...
    protected void timeEventToActors(TimeEvent te) {
        ...
        Set<Actor> matched = keyMap.get(te.event().key());
