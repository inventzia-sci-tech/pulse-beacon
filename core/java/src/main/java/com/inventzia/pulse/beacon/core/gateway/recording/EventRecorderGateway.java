 ...
 * authoritative {@link RunInfo}, starts {@link #run()} on a thread it owns, and later calls {@link
 * #requestStop()} and joins that thread. Header metadata is the engine's {@code RunInfo}, never
 * recomputed here, so the recording agrees with the run's manifest.
 *
 * <p><b>Stage A limitation (selected routes, recorder-local sequence).</b> This is a <em>subscriber</em>
 * gateway, and Beacon routing is one-to-one per {@code (topic, key)}, so it can only be registered on
 * routes that have no other sink. Its {@code seq} is therefore <em>recorder-local</em> (the order it
 * received events on its subscribed routes), not the engine's global dispatch sequence. Both facts are
 * recorded in the run manifest's {@code recording} descriptor, so the viewer never mistakes a partial
 * recording for the complete engine stream. The global sequence and all-routes capture arrive with the
 * engine event tap (Stage B).
 *
 * <p><b>Performance contract.</b> Non-participation does not mean zero cost. {@link #onEvent} runs on
 * the dispatch thread and does the minimum only (count, stamp {@code seq}, capture {@code observedAt}
 * and the immutable datum reference, offer to a bounded queue). Serialization and disk I/O run on the
 * owner's thread ({@link #run()}). Overflow drops with a counter; a serialization failure is counted
...
public class EventRecorderGateway extends AbstractGateway {
    ...
    private BufferedWriter writer;          // writer-thread only
     ...
     * @param runId     run identifier stamped on every record (the run directory name)
     ...
     */
    public EventRecorderGateway(String name, String runId, long startTime, long endTime, int capacity) {
        super(name, startTime, endTime);
        this.runId        = Objects.requireNonNull(runId, "runId");
        this.recorderName = "pulse-beacon/EventRecorderGateway v" + ENVELOPE_VERSION;
    ...
    /**
     * Bind the recorder to its output file and the run's authoritative description. Call once, before
     * starting {@link #run()}.
     */
    public synchronized void configure(Path filePath, RunInfo info) {
        if (started) throw new IllegalStateException(name() + ": already started");
        this.filePath   = Objects.requireNonNull(filePath, "filePath");
        this.headerInfo = Objects.requireNonNull(info, "info");
    }
    ...
    public Counts counts() {
        ...
        return new Counts(obs, written, ovf, ser, abandoned);
    }

    /** @return the recording outcome: {@code "failed"}, {@code "interrupted"}, or {@code "complete"}. */
    public String recordingStatus() {
        return failed ? "failed" : interrupted ? "interrupted" : "complete";
    }

    /** @return {@code true} once the writer thread has run to termination. */
    ...
    @Override
    public void run() {
        if (filePath == null || headerInfo == null) {
            throw new IllegalStateException(name() + ": configure(file, info) must be called before run()");
        }
        started = true;
        try {
            openFile();
    ...
    private void writeHeader() {
        RunInfo info = headerInfo;
        String fp = info.typeFingerprint();
        List<String> providers = info.providerIds();
        ...
        b.append("{\"kind\":\"header\",\"v\":").append(ENVELOPE_VERSION)
         ...
         .append(",\"operatingMode\":").append(str(info.mode().name()))
         .append(",\"window\":{\"start\":").append(info.startTime()).append(",\"end\":").append(info.endTime()).append('}')
    ...
    private void safeWriteTrailer() {
        try {
            ...
            writeLine("{\"kind\":\"trailer\",\"v\":" + ENVELOPE_VERSION
                    + ",\"runId\":" + str(runId)
                    + ",\"status\":" + str(recordingStatus())
                    + ",\"observed\":" + c.observed()
                    + ",\"events\":" + c.events()
                    + ",\"overflow\":" + c.overflow()
                    + ",\"serializationErrors\":" + c.serializationErrors()
