...
_BEACON = _TESTS.parents[1]          # .../pulse-beacon  (tests/ is at the repo root)
...
def jep_launch(require_prereq):
    # A source-tree integration test: the JEP subprocess needs a compiled Java build
    # (target/classes) + jep, which an installed-wheel-only environment does not have.
    # Missing prerequisites skip locally but fail under PULSE_REQUIRE_INTEGRATION.
    prefix = os.environ.get("CONDA_PREFIX")
    if not prefix:
        require_prereq("no CONDA_PREFIX; run inside the `pulse` conda env")
    prefix = Path(prefix)

    libjep = list(prefix.glob("lib/python*/site-packages/jep/libjep.so"))
    if not libjep:
        require_prereq("jep is not installed in this env")
    ...
    if jep_jar is None or not _CLASSES.exists() or not jars:
        require_prereq("build core/java first (target/classes + staged jars/); see README")
